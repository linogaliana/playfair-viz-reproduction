...
```{ojs}
{
  ...
  const img_playfair = Object.assign(new Image(), { src: "https://minio.lab.sspcloud.fr/lgaliana/data/dataviz/chart_national_debt_reproduction.png" });
  img_playfair.style.maxWidth = "100%";
  img_playfair.style.height = "auto";
  left.appendChild(img_playfair);
...
Parmi les principaux problèmes de la représentation de gauche, on peut noter l'axe à droite auquel nous sommes peu familiers[^axe] et le point de départ de l'axe des ordonnées. Sur ce dernier sujet, il est plus juste de partir de 0 qui fournit, en quelques sorte, un idéal (un Etat sans endettement) permettant d'évaluer plus justement les valeurs observées et la pente de la série temporelle.

[^axe]: Ici, il ne me semble pas choquant de représenter à droite l'axe des $y$ si on désire focaliser l'attention sur les dernières valeurs. J'ai choisi de remettre l'axe à gauche pour correspondre aux attentes usuelles des lecteurs. De toute manière, pour connaître les valeurs précises, l'interactivité permet d'afficher celles-ci avec la souris (ordinateur) ou en cliquant sur un point (smartphone ou tablette). 

On peut aussi tirer parti de l'interactivité du navigateur web pour rendre plus ergonomique la visualisation. Pour améliorer la contextualisation de ces statistiques, des annotations sur certaines périodes clés ne seraient pas inutiles. Mais comme elles peuvent alourdir la visualisation sans réflexion approfondie sur l'ergonomie de la figure, j'ai préféré ne pas en mettre.
...
et récupérables sous forme de JSON depuis l'URL [https://api.insee.fr/melodi/data/DD_CNA_APU?REF_SECTOR=S13&STO=_GD_PPIB](https://api.insee.fr/melodi/data/DD_CNA_APU?REF_SECTOR=S13&STO=_GD_PPIB){target="_blank"}. 
...
```{ojs}
Plot = require("https://cdn.jsdelivr.net/npm/@observablehq/plot@0.6.16/dist/plot.umd.min.js")
```
